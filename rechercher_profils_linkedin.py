...
# Mots-clés pour repérer la colonne contenant le NOM de l'entreprise (colonne B) :
# c'est cette colonne qui sert désormais de base à la recherche, plutôt que le nom
# extrait depuis l'URL LinkedIn.
MOTS_CLES_COL_NOM = ["nom entreprise", "nom", "name", "entreprise"]
...
def extraire_nom_entreprise(url):
    """Extrait le nom de l'entreprise depuis l'URL LinkedIn (utilisé uniquement en
    secours si la colonne Nom est absente ou vide pour une ligne donnée)."""
...
def main():
    ...
    # le fichier d'entrée. La colonne Nom (colonne B) sert désormais de base à la
    # recherche ; site/adresse/téléphone ne sont recherchés que si absents.
    col_nom = detecter_colonne(df_entree, MOTS_CLES_COL_NOM)
    ...
    print(f"ℹ️ Colonnes détectées dans le fichier d'entrée — Nom: {col_nom or 'aucune (repli sur URL)'}, "
          f"Site: {col_site or 'aucune'}, Adresse: {col_adresse or 'aucune'}, "
          f"Téléphone: {col_telephone or 'aucune'}")

    # Table de correspondance URL -> infos déjà connues (nom/site/adresse/téléphone),
    ...
    for index, url in enumerate(urls_du_lot, 1):
        ...
        # Le nom d'entreprise vient EN PRIORITÉ de la colonne Nom (colonne B) du fichier
        # d'entrée. Si elle est absente ou vide pour cette ligne, on retombe sur le nom
        # extrait de l'URL LinkedIn (comportement d'origine), pour ne jamais bloquer une
        # ligne si la colonne Nom n'a pas été renseignée.
        infos_connues = infos_existantes.get(url_clean, {})
        nom_entreprise = (infos_connues.get("nom") or "").strip() or extraire_nom_entreprise(url_clean)

        if not nom_entreprise:
            print(f"[{index}/{len(urls_du_lot)}] Nom d'entreprise introuvable (colonne Nom vide et URL non valide) : {url_clean}")
